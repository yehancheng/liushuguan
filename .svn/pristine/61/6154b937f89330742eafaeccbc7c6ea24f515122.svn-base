...
function getStockDescWords(state,isPurchase,skuid,skukey,arrivalDate,isNotice,ext,provinceId,dti,rn,rfg,pr){
	...
	if (state == 33){
		...
		if (rn&&rn>0){
			text = "<strong>有货</strong>，仅剩"+rn+"件"+(promiseresult?("，"+promiseresult):"");	
		}
		else{
			text = "<strong>有货</strong>，"+(promiseresult?promiseresult:"下单后立即发货");		
		}
	}
	else if (state == 34 || state == 0){
		text = "<strong>无货</strong>，该商品暂时售完"+(isNotice?"，<a id='notify-stock' data-type='2' data-sku='"+skuid+"' href='http://notify.home.jd.com/email.action?type=2&id=" + skuid + "&key=" + skukey + "' target='_blank'>到货通知</a>":"");
		if (skuid&&skuid.length == 8 && !isPurchase){
			text = "<strong>无货</strong>，此商品不再销售，欢迎选购其它商品";
		}
	}
	else if (state == 39){
		text = "<strong>有货</strong>，下单后2-6天发货";
	}
	else if (state == 40){
		text = "<strong>有货</strong>，下单后2-6天发货";
		if (rfg==2){
			text = "<strong>有货</strong>，预计下单后5-10天发货";
		}
		else if (dti&&dti.fastestDays&&dti.slowestDays){
			text = "<strong>有货</strong>，下单后"+dti.fastestDays+"-"+dti.slowestDays+"天发货";
		}
	}
	else if (state == 36){
		text = "<strong>预订</strong>，"+(arrivalDate?"预计"+arrivalDate+"日后有货，现在可下单":(rfg==2?"下单后5-7周发货":"商品到货后发货，现在可下单"));
	}
	text += yfInfo;
	return text;
}
...
function gSC(result) {
	if (result.stock) {
		...
		if (result.stock.StockState == 34 || result.stock.StockState == 0){ //商品无货
            pageConfig.product.haveStock = false;
			$("#choose-btn-easybuy").addClass("hide");
			$("#InitCartUrl").attr("href","#none").attr("onclick","").attr("title","商品已无货").unbind("click");
            ...
            if ( $('#choose-noresult').length < 1 && $('#itemover-title').length < 1 ) {
				if (result.stock.IsPurchase){
					$('#choose-btns').before('<li id="choose-noresult"><div class="dd"><strong>所选地区该商品暂时无货！</strong></div></li>');
				}
				else{
					$('#choose-btns').before('<li id="choose-noresult"><div class="dd"><strong>此商品不再销售，欢迎选购其它商品！</strong></div></li>');
...
function ShowStockForAll(ipLoc,writeCookie){//获取地区库存
	if(!ipLoc){
				...
				success:function(r){
					...
					if(r.Flag){
						if($("#choose-btn-easybuy .btn-easybuy").length==0){
							...
							$("<div id=\"choose-btn-easybuy\" class=\"btn\"><a href=\"#none\" class=\"btn-easybuy\">轻松购<b></b></a></div>").insertAfter("#choose-btn-append");
						}
						else{
							$("#choose-btn-easybuy").removeClass("hide");
						}
